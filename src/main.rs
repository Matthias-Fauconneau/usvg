use {ui::{widget::{Widget, Target, size, EventContext, Event}, app::run}, fehler::throws, anyhow::Error, ::xy::xy};

struct PagesView{ pages: Vec<usvg::Tree>, skip: usize }

impl Widget for PagesView {
	...
	#[throws] fn paint(&mut self, target: &mut Target) {
		...
		for page in self.pages.iter().skip(self.skip) {
			...
			let page = image::Image{stride: page.width(), size: xy{x: page.width(), y: page.height()}, data: unsafe{image::slice::cast::<rgba8>(page.data())}};
	...
	#[throws] fn event(&mut self, _: size, _: &EventContext, event: &Event) -> bool {
